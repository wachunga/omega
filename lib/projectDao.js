...
projectDao.remove = function (slug) {
	return projectDao.doRemove(slug, true);
};

projectDao.unremove = function (slug) {
	return projectDao.doRemove(slug, false);
};

projectDao.doRemove = function (slug, value) {
	var project = projects[slug];
	if (!project) {
		return false;
	}
	projects[slug].deleted = value;
	this.write();
	return true;
};

projectDao.toggleUnlisted = function (slug) {
	var project = projects[slug];
	if (!project) {
		return false;
	}
	projects[slug].unlisted = !project.unlisted;
	this.write();
	return true;
};
