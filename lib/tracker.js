...
var allowedHtml = 'a|b|blockquote|code|del|em|i|pre|sup|sub|strong|strike';
...
function markdownizeItem(item) {
	_.each(['description', 'message'], function (property) {
		if (item[property]) {
			item[property] = markdown(item[property], true, allowedHtml, null, true);
		}
	});
	return item;
}
...
function onConnect(project, socket, projectSocket) {
	...
	socket.on('user message', function (msg) {
		var event = markdownize(recordEvent(ET.UserMessage, {message: msg, speaker: socket.nickname}));
